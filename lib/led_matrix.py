...
class LedMatrix:
            ...
            for pin in row:
                if hasattr(pin, 'on') and hasattr(pin, 'off') and hasattr(pin, 'toggle'):
                    ...
                    if hasattr(shift_register, 'set_pin'):
                        ...
                        led_row.append(ShiftRegisterLed(**led_params))
                    else:
                        raise ValueError(f'Invalid shift register pin: {pin}')
                else:
                    # No longer accepting raw pin numbers
                    error_msg = (
                        f'Invalid pin type: {pin}. Must provide either an LED object with '
                        f'on/off/toggle methods or a (shift_register, position) tuple.'
                    )
    ...
    def animate_snake(self):
        ...
        async def animation_step():
            ...
            def pattern(row, col):
                ...
                return (
                    pos == head_pos
                    or pos == ((head_pos - 1) % total_positions)
                    or pos == ((head_pos - 2) % total_positions)
                )
    ...
    def cycle_animation(self):
        ...
        try:
            ...
            print(
                f"Warning: Animation '{self.current_animation}' not in ANIMATION_STATES. "
                f"Resetting to 'blink'."
            )
            self.current_animation = 'blink'
