...
BOARDS = {
    ...
    'esp32-c3': {
        ...
        'chip': 'esp32c3',
        'port': '/dev/tty.usbmodem1101',  # /dev/tty.usbmodem101
        'baud': '460800',
        'flash_mode': 'dio',
        'flash_addr': '0x0',
...
def main():
    ...
    parser.add_argument(
        '--board', '-b', choices=list(BOARDS.keys()), default='esp32', help='Board type to flash'
    )
    parser.add_argument('--port', '-p', help='Serial port (overrides board default)')
    parser.add_argument(
        '--firmware',
        '-f',
    ...
    if not os.path.exists(firmware):
        ...
        print('Download the appropriate firmware for your board:', file=sys.stderr)
        print('ESP32: https://micropython.org/download/esp32/', file=sys.stderr)
        print('ESP32-C3: https://micropython.org/download/esp32c3/', file=sys.stderr)
        return 1
